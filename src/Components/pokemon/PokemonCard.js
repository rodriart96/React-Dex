...
export default class PokemonCard extends Component {
    ...
    componentDidMount(){
        ...
        const dexNumber = url.split("/")[url.split('/').length - 2];
        const img = `https://github.com/PokeAPI/sprites/blob/master/sprites/pokemon/${dexNumber}.png?raw=true`
    ...
    render() {
        return (
            <div className='col-md-4 col-sm-6 col-xs-12 mb-5'>
                <StyledLink to={`pokemon/${this.state.dexNumber}`}>
                <Card className='card'>
                    <h5 className='card-header'> {this.state.dexNumber} </h5>
